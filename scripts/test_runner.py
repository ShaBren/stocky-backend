...
def run_tests(
    category: str = "all", coverage: bool = False, verbose: bool = False
) -> int:
    ...
    if coverage:
        base_cmd += (
            " --cov=src/stocky_backend --cov-report=html --cov-report=term-missing"
        )
...
def setup_test_environment() -> int:
    ...
    commands = ["pip install -r tests/requirements-test.txt", "pip install -e ."]
