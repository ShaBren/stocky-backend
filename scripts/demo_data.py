    ...
    sample_items = [
        {
            ...
            "upc": "012345678901",
            "default_storage_type": StorageType.REFRIGERATOR,
        },
        {
            "name": "Bread - Whole Wheat",
            "description": "Whole wheat sandwich bread",
            "upc": "012345678902",
            "default_storage_type": StorageType.PANTRY,
        },
        {
            "name": "Chicken Breast",
            "description": "Boneless skinless chicken breast",
            "upc": "012345678903",
            "default_storage_type": StorageType.FREEZER,
        },
        {
            "name": "Bananas",
            "description": "Fresh bananas",
            "upc": "012345678904",
            "default_storage_type": StorageType.COUNTER,
        },
        {
            "name": "Rice - Jasmine",
            "description": "Jasmine white rice, 5lb bag",
            "upc": "012345678905",
            "default_storage_type": StorageType.PANTRY,
        },
        {
            "name": "Eggs - Large",
            "description": "Large white eggs, dozen",
            "upc": "012345678906",
            "default_storage_type": StorageType.REFRIGERATOR,
        },
        {
...
def create_sample_skus(
    db: Session, admin_user: User, items: list[Item], locations: list[Location]
) -> list[SKU]:
    ...
    for item in items:
        ...
        if not appropriate_location:
            # Fallback to pantry
            appropriate_location = next(
                (loc for loc in locations if loc.storage_type == StorageType.PANTRY),
                locations[0],
            )
        ...
        existing_sku = (
            db.query(SKU)
            .filter(SKU.item_id == item.id, SKU.location_id == appropriate_location.id)
            .first()
        )
...
def create_sample_alerts(db: Session, admin_user: User, skus: list[SKU]) -> None:
    ...
    soon_expiring_skus = [
        sku
        for sku in skus
        if sku.expiry_date and sku.expiry_date <= datetime.now() + timedelta(days=3)
    ][:2]
