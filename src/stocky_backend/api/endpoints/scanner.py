...
async def associate_scanner(
    association: ScannerAssociation, current_user=Depends(require_user_role())
):
...
async def disassociate_scanner(scanner_id: str, current_user=Depends(require_user_role())):
    ...
    if scanner_id in scanner_associations:
        ...
        raise HTTPException(
            status_code=404, detail=f"Scanner {scanner_id} is not currently associated"
        )
...
async def list_scanner_associations(current_user=Depends(require_user_role("manager"))):
    ...
    return {"associations": scanner_associations, "count": len(scanner_associations)}
