...
### Fixed
- **Docker**: Removed deprecated `ALGORITHM` and `ACCESS_TOKEN_EXPIRE_MINUTES` env vars from compose
...
### Fixed
- **Docker**: Copy `pyproject.toml` to runtime image (was missing, broke version display)
- **Docker**: Make `alembic upgrade head` idempotent for upgrades from v0.2.4
  - Detects existing databases without `alembic_version` table and stamps head
  - Handles fresh databases, existing-with-history, and existing-without-history
